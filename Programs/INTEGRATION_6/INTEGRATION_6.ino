...
void loop()
{
  ...
  if (num != -1) 
  {
    ...
    switch (key) 
    {
     ...
     {
      Serial.print("Delta2 >1000");
      //ALARM_ON (ALARM_LED1);
      //AUTOSHUTOFF();
      //Serial.print("Auto Shutoff executed");
     ALERT(); //NEEDS ANALOG READ 
     }
     //else
     //{
      //ALARM_OFF (ALARM_LED1);
     //}

     if (((CRV <= X) & (CRV >= (X - 5))) | ((PRV <= Z) & (PRV >= (Z - 5))))
     {
      ALARM_ON (ALARM_LED4);  //reservoir low/preset reservoir level about to reach
      Serial.print("CRV=");
      Serial.print(CRV);
      Serial.print("PRV=");
      Serial.print(PRV);
...

{
  ...
  {
    ALARM_ON(ALARM_LED1);  // drip stopped
  }
  else
  {
    ALARM_OFF(ALARM_LED1);
...
void ALARM_ON (int p)  // function
{
    digitalWrite(p, HIGH);
    alarm_on = true;
}

void ALARM_OFF(int p)  // function
{
  digitalWrite(p, LOW);
  alarm_on = false;
}

void AUTOSHUTOFF()  // function
{
