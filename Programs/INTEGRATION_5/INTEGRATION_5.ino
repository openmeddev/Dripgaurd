...
float X = 0;// used for alarms 
float Y = 0;// used for alarms 
float Z = 0;// used for alarms 
int truevalue1 = 0, truevalue2 = 0; // for analog read of LDR during refresh
int truevaluenow1 = 0, truevaluenow2 = 0;
boolean alarm_on = false;
boolean start = false;

boolean ALARM_LED1_STATE = false;
boolean ALARM_LED2_STATE = false;
boolean ALARM_LED3_STATE = false;
boolean ALARM_LED4_STATE = false;
boolean ALARM_LED5_STATE = false;
boolean ALARM_LED6_STATE = false;
boolean ALARM_LED7_STATE = false;
boolean ALARM_LED8_STATE = false;
...
void loop()
{
  CRV = (IRV - (DC/DF)); // Current reservoir volume
  
  X = ((1.0/10.0)*IRV); // MOVE to
  Y = ((1.0/20.0)*IRV);
  Z = ((1.0/20.0)*PRV); 
  ...
  if (num != -1) 
  {
    ...
    switch (key) 
    {
      ...
      if ((num <= 1000) && (num >= 50)) 
      {
        IRV = num;
        PRV = IRV;
        Serial.println("Input Reservoir Volume:");
        Serial.println(IRV);
        Serial.println("IRV is Equal to PRV in this case");
        Serial.println("Preset Reservoir Volume");
        Serial.println(PRV);
       }
       else 
       {
       ...
       {
        IT = num;
        Serial.println("Infusion Time:");
        Serial.println(IT);
        IDR = ((IRV*DF)/IT);  // drip rate calculatio
        Serial.println("Input Drip Rate:");
        Serial.println(IDR);
       }
       else 
       {
       ...
       {
        PRV = num;
        Serial.println("Preset Reservior Volume:");
        Serial.println(PRV);
       }
       else 
       {
     ...
     if (((CRV <= X) & (CRV >= (X - 5))) | ((PRV <= Z) & (PRV >= (Z - 5))))
     {
      ALARM(ALARM_LED4);
      ALARM_LED4_STATE = true;
      Serial.print("CRV=");
      Serial.print(CRV);
      Serial.print("PRV=");
      Serial.print(PRV);
     ...
     {
      alarm_on= false;
      Serial.println("Truning alarm off");
     }
     ...
     //ALARM_LED4_STATE = false;
     ALARM_OFF(ALARM_LED5); 
     }
...
int numinput() // function
{
  ...
  while (true) 
  {
    ...
    {
      Serial.println("Exiting key input");
      Serial.println(key2);
      return (num1);
      break;
    }
...
{
  truevaluenow1 = analogRead(LDR1);
  truevaluenow2 = analogRead(LDR2);
  
  if (((truevaluenow1 < (truevalue1 - 50))|(truevaluenow1 > (truevalue1 + 50))) && ((truevaluenow2 < (truevalue2 - 50))|(truevaluenow2 > (truevalue2 + 50))))
  {
    ALARM(ALARM_LED1);  // drip stopped
    ALARM_LED1_STATE = true;
  }

  if (((truevaluenow1 < (truevalue1 - 50))|(truevaluenow1 > (truevalue1 + 50))) && ((truevaluenow2 < (truevalue1 - 50))|(truevaluenow2 > (truevalue1 + 50))))
  {
    ALARM(ALARM_LED2);  // drip chamber empty
    ALARM_LED2_STATE = true;
  }

  if (((truevaluenow1 < (truevalue2 - 50))|(truevaluenow1 > (truevalue2 + 50))) && ((truevaluenow2 < (truevalue2 - 50))|(truevaluenow2 > (truevalue2 + 50))))
  {
    ALARM(ALARM_LED3);  //  drip chamber full
    ALARM_LED3_STATE = true;
  }
}
...
void CALIBERATE ()     //  function
{
  ...
  DC = 0;
  MDR = 0;
  ALARM_LED1_STATE = false;
  ALARM_LED2_STATE = false;
  ALARM_LED3_STATE = false;
  ALARM_LED4_STATE = false;
  ALARM_LED5_STATE = false;
  ALARM_LED6_STATE = false;
  ALARM_LED7_STATE = false;
  ALARM_LED8_STATE = false;
  //TBC: open the valve
}
